-- 1. Fix request_fighter_license function with proper validations and correct license_documents insertion
...
BEGIN
  ...
  v_first_name := NULLIF(TRIM(p_fighter_profile_data->>'first_name'), '');
  v_last_name := NULLIF(TRIM(p_fighter_profile_data->>'last_name'), '');
  v_weight_class := NULLIF(TRIM(p_fighter_profile_data->>'weight_class'), '');

  IF v_first_name IS NULL OR v_last_name IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'message', 'Debes completar tu nombre y apellido en tu perfil'
    );
  END IF;
  ...
  -- Create fighter profile with safe casting
...
-- 2. Increase identity_documents bucket size limit to 50MB
...
-- 3. Create fighter-avatars bucket if it doesn't exist
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'fighter-avatars',
  'fighter-avatars',
  true,
  10485760,  -- 10MB for avatars
...
-- 4. RLS policies for fighter-avatars bucket
