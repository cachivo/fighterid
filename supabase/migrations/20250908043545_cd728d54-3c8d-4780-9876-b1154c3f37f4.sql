-- Create new admin function without elo_rating references
CREATE OR REPLACE FUNCTION public.admin_update_fighter_profile_v4(p_fighter_id uuid, p_profile_data jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SET 
    ...
    record_wins = CASE 
      WHEN p_profile_data ? 'record_wins' THEN
        COALESCE((p_profile_data->>'record_wins')::integer, record_wins)
      ELSE record_wins
    END,
    record_losses = CASE 
      WHEN p_profile_data ? 'record_losses' THEN
        COALESCE((p_profile_data->>'record_losses')::integer, record_losses)
      ELSE record_losses
    END,
    record_draws = CASE 
      WHEN p_profile_data ? 'record_draws' THEN
        COALESCE((p_profile_data->>'record_draws')::integer, record_draws)
      ELSE record_draws
    END,
  ...
  -- Handle discipline separately with proper validation and casting
...
DROP FUNCTION IF EXISTS public.admin_update_fighter_profile_v2(uuid, jsonb, uuid);
DROP FUNCTION IF EXISTS public.admin_update_fighter_profile_v3(uuid, jsonb);