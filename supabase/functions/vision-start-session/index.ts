...
interface ConnectSessionPayload {
...
serve(async (req) => {
  ...
  try {
    ...
    if ((path === 'vision-start-session' || path === 'start-session') && req.method === 'POST') {
      const body: StartSessionPayload = await req.json().catch(() => ({}));
      ...
      if (error) {
        console.error('Error creating vision session:', error);
        return new Response(
      ...
      return new Response(
        JSON.stringify({
          ...
          hud_connected: data.hud_connected,
          vision_connected: data.vision_connected,
    ...
    if (path === 'connect' && req.method === 'POST') {
      const body: ConnectSessionPayload = await req.json().catch(() => ({}));
      ...
      const updatePayload: { vision_connected: boolean; fight_id?: string | null } = {
        vision_connected: true,
      };

      if (body.fight_id) {
        updatePayload.fight_id = body.fight_id;
      }
