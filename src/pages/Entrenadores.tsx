...
export default function Entrenadores() {
  ...
  const filteredCoaches = coaches?.filter(coach => 
    ...
    coach.ciudad?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-primary/5 via-secondary/5 to-accent/5 border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 pt-20">
          <PageHeader
            title="Entrenadores"
            subtitle="Conoce a los mejores entrenadores de artes marciales"
            backTo="/"
            backLabel="Volver al inicio"
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-medium">
              <Users className="h-4 w-4" />
              {coaches?.length || 0} Entrenadores Registrados
            </div>
          </PageHeader>
        </div>
      </div>
